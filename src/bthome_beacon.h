...
typedef struct __attribute__((packed)) {
	...
	u16	battery_mv; // mV
#endif
} seg_bthome_data_t, * pseg_bthome_data_t; // max 15 bytes!

/* Segment events */
typedef struct __attribute__((packed)) _adv_bthome_event1_t {
	u8	o_id;	// = BtHomeID_opened ?
	u8	opened;
	u8	c_id;	// = BtHomeID_count32
	u32	counter;
} seg_bthome_event_t, * pseg_bthome_event_t; // max 15 bytes!

// BTHOME data, no security
typedef struct __attribute__((packed)) _adv_bthome_ns1_t {
	...
	seg_bthome_data_t data; // max 28 - 7 = 21 bytes
} adv_bthome_data_nc_t, * padv_bthome_data_nc_t; // max 31 - 3(BLE flags) = 28 bytes!

// BTHOME event, no security
typedef struct __attribute__((packed)) _adv_bthome_ns_ev1_t {
	adv_head_bth_t head;
	u8	info;	// = 0x40 BtHomeID_Info
	u8	p_id;	// = BtHomeID_PacketId
	u8	pid;	// PacketId (!= measurement count)
	seg_bthome_event_t data;
} adv_bthome_event_nc_t, * padv_bthome_event_nc_t;
...
// BTHOME data, security
typedef struct __attribute__((packed)) _adv_bthome_d1_t {
	adv_head_bth_t head; // 4 bytes
	u8	info;	 // = 0x41 BtHomeID_Info_Encrypt
	seg_bthome_data_t data; // max 28 - 13 = 15 bytes
	u32	count_id; //  4 bytes
	u8	mic[4]; //  4 bytes
} adv_bthome_data_en_t, * padv_bthome_data_en_t; // max 31 - 3(BLE flags) = 28 bytes!

// BTHOME event, security
typedef struct __attribute__((packed)) _adv_bthome_ev1_t {
	...
	u32	count_id;
	u8	mic[4];
} adv_bthome_event_en_t, * padv_bthome_event_en_t; // flag[3]+h[13]+data[x]
