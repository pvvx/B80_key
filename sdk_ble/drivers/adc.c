...
void adc_set_ref_voltage(ADC_RefVolTypeDef v_ref)
{
	...
	if(v_ref == ADC_VREF_1P2V)
	{
		...
		analog_write( areg_ain_scale  , (analog_read( areg_ain_scale  )&(FLD_SEL_AIN_SCALE))
			| 0x3d);
			// FLD_ADC_ITRIM_PREAMP(1) | FLD_ADC_ITRIM_VREFBUF(3) | FLD_ADC_ITRIM_VCMBUF(3)
	}
	else
	{
		...
		analog_write( areg_ain_scale  , (analog_read( areg_ain_scale  )&(FLD_SEL_AIN_SCALE))
			| 0x15);
			//  FLD_ADC_ITRIM_PREAMP(1) | FLD_ADC_ITRIM_VREFBUF(1) | FLD_ADC_ITRIM_VCMBUF(1));
...
extern u32 rnd_m_w, rnd_m_z;
extern u32 adc_rng_result(void);
